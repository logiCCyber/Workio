...
class _WorkerChatScreenState extends State<WorkerChatScreen> {
  ...
  Future<void> _showAttachSheet() async {
    if (_sending || widget.isViewOnly) return;
  ...
  Future<void> _markReadSafe() async {
    ...
    try {
      await MessageService.markThreadRead(_threadId);
    } catch (_) {}
  }
  ...
  String _headerStatusText() {
    ...
    return _peerOnline ? 'Online' : 'Offline';
  }

  bool _isIncomingForDivider(String senderRole) {
    return senderRole.toLowerCase() != 'worker';
  }
  ...
  void _captureUnreadBoundary(List<Map<String, dynamic>> messages) {
    ...
    for (final msg in messages) {
      ...
      if (senderRole != 'worker' && readAt == null && msgId.isNotEmpty) {
  ...
  Future<void> _send() async {
    ...
    if (text.isEmpty || _sending || _threadId.isEmpty) return;

    setState(() => _sending = true);

    try {
      await _sendActivity('typing', active: false);
      await MessageService.sendWorkerMessage(
        threadId: _threadId,
        text: text,
  ...
  Future<void> _sendImageFromGallery() async {
    if (widget.isViewOnly || _sending || _threadId.isEmpty) return;
    ...
    try {
      ...
      await MessageService.sendWorkerImageMessage(
        threadId: _threadId,
        file: file,
        replyToMessageId: _s(_replyToMessage?['id']),
      );
  ...
  Future<void> _sendFile() async {
    ...
    try {
      ...
      await MessageService.sendWorkerFileMessage(
        threadId: _threadId,
        file: file,
        replyToMessageId: _s(_replyToMessage?['id']),
      );
  ...
  Future<void> _pinMessage(Map<String, dynamic> msg) async {
    ...
    if (messageId.isEmpty || _threadId.isEmpty) return;
  ...
  Future<void> _showMessageActions(
    ...
    final isMine = _s(msg['sender_role']).toLowerCase() == 'worker';
    ...
    switch (selected) {
      ...
      case _MessageAction.edit:
        await _showEditMessageDialog(msg);
        break;
  ...
  Future<void> _showEditMessageDialog(Map<String, dynamic> msg) async {
    ...
    if (result.trim() == currentText.trim()) return;
  ...
  Future<void> _deleteMessage(Map<String, dynamic> msg) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'This message will be hidden and shown as deleted.',
          style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    final worker = _thread?['workers'] is Map
        ? Map<String, dynamic>.from(_thread!['workers'] as Map)
        : <String, dynamic>{};

    final workerName = _s(worker['name']).isEmpty ? 'Admin chat' : _s(worker['name']);
    final workerEmail = _headerStatusText();
    final avatarUrl = '';
          ...
          SafeArea(
            child: Column(
              children: [
                ...
                SizedBox(height: _threadId.isNotEmpty ? 6 : 12),
                if (_threadId.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(12, 0, 12, 10),
                    child: StreamBuilder<Map<String, dynamic>?>(
                      stream: MessageService.watchPinnedMessage(_threadId),
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'View-only mode: chat is read-only.',
                              style: TextStyle(
                    ...
                    children: [
                      Positioned.fill(
                        child: _loading
                          ...
                          stream: MessageService.watchMessages(_threadId),
                          builder: (context, snapshot) {
                            final messages =
                            ...
                            return ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                if (showUnreadDivider) {
                                  debugPrint('WORKER SHOW unread divider for msgId=${_s(msg['id'])}');
                                }

                                final senderRole =
                                _s(msg['sender_role']).toLowerCase();
                                final isMine = senderRole == 'worker';
                                final isDeleted = _s(msg['deleted_at']).isNotEmpty;
                                final isEdited = _s(msg['edited_at']).isNotEmpty;
                                final createdAt = _parseDate(msg['created_at']);
                                final prevCreatedAt = prev == null
                                    ? null
                                    : _parseDate(prev['created_at']);
                                  ...
                                  children: [
                                    ...
                                    Align(
                                      ...
                                      child: GestureDetector(
                                        onLongPressStart: (!isDeleted && !widget.isViewOnly)
